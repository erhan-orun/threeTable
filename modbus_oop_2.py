...
class ModbusOop(object):
    def __init__(self):
        self.regs_count = len(cnf.cnfOperation.readSensorNo_2())
        self.root = tk.Tk()
    ...
    def on_double_click(self, event):
        item = self.tree_2.identify('item', event.x, event.y)

        print(self.tree_2.item(item, "text"))
        ...
        xs_doc = list(
            mycol.find(
                {"$and": [{"Sensor No": self.tree_2.item(item, "text")},
