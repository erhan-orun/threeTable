...
class ConnectModbus():
    ...
    def connect_modbus():
        ...
        regNoList = []
        resultList = []
        for i in cnf.cnfOperation.readSensorNo_2():
            host = cnf.cnfOperation.readModBusHost()
            groupNo = math.floor(((int(cnf.cnfOperation.readLineNo_2()) - 1) / 256)) + 1
            portNo = 10000 + (int(cnf.cnfOperation.readSensorTypeNo_2()) - 1) * 10 + groupNo - 1
            regNo = (((int(cnf.cnfOperation.readLineNo_2()) - 1) * 128 + (int(i) - 1)) * 2) % 65536
